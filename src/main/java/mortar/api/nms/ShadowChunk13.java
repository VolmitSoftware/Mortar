...
public class ShadowChunk13 implements ShadowChunk
{
	...
	public ShadowChunk13(org.bukkit.Chunk chunk)
	{
		this.chunk = chunk;
	...
	{
		setBiome(x, z, IRegistry.BIOME.a(CraftBlock.biomeToBiomeBase(bio)));
	}

	@Override
	public void setBiome(int x, int z, int id)
	{
		nmsCopy.getBiomeIndex()[(z & 15) << 4 | x & 15] = IRegistry.BIOME.fromId(id);
		biomeModified = true;
	}
	...
	{
		nmsCopy = new Chunk(actual.world, new ProtoChunk(actual.getPos().x, actual.getPos().z, actual.F()), actual.getPos().x, actual.getPos().z);
		for(int i = 0; i < 16; i++)
		{
			rebaseSection(i);
		}
